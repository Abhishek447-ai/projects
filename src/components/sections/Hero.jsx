import { motion } from "framer-motion"
import sun from "../../assets/sun.png"

export default function Hero() {
  return (
    <section
      className="
        relative
        min-h-[100svh]
        flex items-center justify-center
        overflow-hidden
        px-4 md:px-6
      "
    >
      {/* Base gradient */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,#fff8ef,#f7f4ef)]" />

      {/* Decorative blobs */}
      <div className="absolute top-[-150px] left-[-150px] w-[300px] md:w-[600px] h-[300px] md:h-[600px] rounded-full blur-[120px] md:blur-[180px] opacity-40 bg-[var(--primary)]" />
      <div className="absolute right-[-150px] bottom-[-150px] w-[300px] md:w-[650px] h-[300px] md:h-[650px] rounded-full blur-[120px] md:blur-[200px] bg-[#f2e7d8] opacity-60" />

      {/* Content */}
      <div className="relative w-full max-w-7xl mx-auto text-center pt-[72px] md:pt-28 px-2">
        <motion.div
          initial={{ opacity: 0, y: 40 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 1 }}
        >
          <div className="flex flex-col items-center">

            {/* Force single line with nowrap */}
            <h1
              className="
                font-black
                leading-[1]
                tracking-tight
                text-[#5e4d3f]
                whitespace-nowrap
                text-[11.5vw] sm:text-[10vw] md:text-[8.25rem]
              "
            >
              Rising{" "}
              <span className="inline-flex items-center">
                S
                <img
                  src={sun}
                  alt="sun"
                  className="
                    inline-block
                    w-[7.5vw] sm:w-[7vw] md:w-[5.9rem]
                    mx-[-1px] md:mx-[-5px]
                    translate-y-[1px]
                    object-contain
                  "
                />
                <span>N</span>
              </span>
            </h1>

            <h2
              className="
                font-black
                text-[#cbb094]
                leading-none
                mt-0 md:mt-[-8px]
                whitespace-nowrap
                text-[13vw] sm:text-[11vw] md:text-[8.5rem]
              "
            >
              Tech Hub
            </h2>
          </div>

          <p className="mt-8 md:mt-10 uppercase tracking-[4px] sm:tracking-[10px] text-[10px] sm:text-sm md:text-lg text-[var(--muted)]">
            Innovate · Build · Rise
          </p>

          <p className="mt-6 md:mt-8 max-w-[300px] sm:max-w-[550px] md:max-w-4xl mx-auto text-[14px] sm:text-lg md:text-2xl leading-7 md:leading-10 text-[var(--muted)]">
            Building elegant digital products, startup platforms and modern
            engineering experiences.
          </p>
        </motion.div>
      </div>
    </section>
  )
}